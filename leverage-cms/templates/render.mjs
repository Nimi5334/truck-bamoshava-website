#!/usr/bin/env node
/**
 * Leverage CMS — build-time renderer (shared across all client sites).
 *
 * Usage:  node leverage-cms/templates/render.mjs <siteDir>
 *
 * Reads   <siteDir>/template.html   (the static shell: head, CSS, scripts)
 *         <siteDir>/content/site.json  (the editable content model)
 * Writes  <siteDir>/index.html      (a fully-populated static page)
 *
 * The renderer is generic and keyed by section `type`, so onboarding a new
 * site is "add content + a template", not "write code". All client-supplied
 * text is HTML-escaped here (defense in depth alongside the Worker's
 * validation) and dietary tags map to a fixed icon vocabulary — never raw
 * client HTML — so the content model can never inject script.
 *
 * Node-only modules are imported lazily inside main() so the pure render()
 * function can also be imported in the browser (the admin live preview).
 */
...
const SOCIAL_ICONS = { instagram: SVG_INSTAGRAM, facebook: SVG_FACEBOOK };
...
function renderHero(s, site) {
  ...
  return `<section class="hero" id="${escAttr(s.id)}" aria-label="${escAttr(site.brand.name)}">
...
function renderMedia(s) {
  ...
  <video id="farmScene" autoplay loop muted playsinline preload="auto"
         ...
         aria-label="${escAttr(d.videoLabel || "")}" disablepictureinpicture>
...
function render(site, template) {
  ...
  return template
    .replace(/\{\{LANG\}\}/g, escAttr(site.meta.lang || "en"))
    .replace(/\{\{DIR\}\}/g, escAttr(site.meta.dir || "ltr"))
...
async function main() {
  ...
  const site = JSON.parse(readFileSync(join(dir, "content", "site.json"), "utf8"));
  const html = render(site, template);
  writeFileSync(join(dir, "index.html"), html, "utf8");
  console.log(`[render] wrote ${join(siteDir, "index.html")} (${html.length} bytes)`);
}
